...
result_path = "./res"
log_id = "test"
load_type = "/home/rjiangas/cv_project/MS-Diffusion/models/doge1516/MS-Diffusion/ms_adapter.bin"
ms_ckpt = f"/home/rjiangas/cv_project/MS-Diffusion/models/doge1516/MS-Diffusion/ms_adapter.bin"
...
def poster_generate(image_path,prompt,phrases,save_name,save_path):
    ...
    save_name = "example_shoes"
    save_path = os.path.join(result_path, save_name)
    os.makedirs(save_path, exist_ok=True)
    for i, image in enumerate(images):
        image.save(os.path.join(save_path, f"{i}.jpg"))
        print("Saving in ",os.path.join(save_path, f"{i}.jpg"))

if __name__ == '__main__':
    image_path = 'example/snaker.jpeg'
    prompt = "This is a poster for sneakers, with the shoes in the center and a playground in the background."
    phrases = [["boots"]]
    save_name = 'boots'
    save_path = 'example'
    slogans = poster_generate(image_path = image_path,prompt=prompt,phrases=phrases,save_name=save_name,save_path=save_path)